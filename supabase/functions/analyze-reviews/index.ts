  ...
  for (let page = 1; page <= 3 && realReviews.length < 10; page++) {
    try {
      ...
      const reviewsResponse = await fetch(
        `https://real-time-amazon-data.p.rapidapi.com/product-reviews?asin=${asin}&country=US&page=${page}&sort_by=TOP_REVIEWS&star_rating=ALL&verified_purchases_only=true&images_or_videos_only=true&current_format_only=false`,
        {
          method: 'GET',
          headers: {
            'x-rapidapi-host': 'real-time-amazon-data.p.rapidapi.com',
